...
- Одно активное WS-соединение; при реконнекте старое принудительно закрывается (защита от дублей)
...
**Сообщение для text-вопроса (1 правильный ответ):**
```
⏱ Время пошло!
...
**Сообщение для text-вопроса (несколько правильных ответов):**
```
⏱ Время пошло!

❓ Вопрос

Назовите 3 страны Латинской Америки.

✏️ Перечисли 3 ответа через запятую.
```

> Если вопрос ожидает N ответов, бот подсказывает сколько нужно. API принимает только первые N вариантов из ответа команды.
