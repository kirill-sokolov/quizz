...
**Валидация:**
- `answerText` — обязательное непустое поле
- Для `choice` вопросов: нормализуется в верхний регистр, допустимы только буквы A–H; иначе `400`
- Для `text` вопросов: обрезается до 500 символов; если правильных ответов несколько (N), принимаются только первые N вариантов пользователя (разделённых `,` или переносом строки)

**Побочные эффекты:**
- Для `text` вопросов: LLM-оценка запускается **немедленно в фоне**. После завершения обновляется `awardedScore` и бродкастится `answer_scored` по WebSocket.

**Response:** `201` — сохранённый объект ответа (без `awardedScore` — он придёт позже через WS)

**Ошибки:** `400` — невалидный ответ; `404` — вопрос не найден; `409` — команда уже ответила
