...
**Реакция:**
- **Admin**: обновить список ответов для текущего вопроса

---

### `answer_scored`
LLM оценил текстовый ответ (приходит через 1–3 сек после `answer_submitted`).
...
**Реакция:**
- **Admin**: обновить отображаемый балл рядом с ответом команды

**Примечание:** событие приходит только для `text`-вопросов. Для `choice` оценка мгновенная и не требует отдельного события.
