...
Порядок слайдов определяется полем `sort_order` в таблице `slides`. Пример:

```
video_warning → video_intro → question → extra → timer → extra → answer → extra
   (optional)     (optional)            (шутка)          (юмор)          (фото)
```

Экстра-слайды (`extra`) могут быть в любой позиции внутри вопроса. Навигация ◀▶ проходит по `sort_order`.
...
- Кнопка ▶ заблокирована до тех пор, пока не все команды ответили
- **Обязательный**
...
### `extra`
- Произвольный дополнительный слайд (шутки, мемы, фото)
- Полноэкранное изображение или видео
- **Опциональный**, может быть несколько в любых позициях
