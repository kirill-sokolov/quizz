...
**Порядок сортировки файлов**: числовая сортировка по имени (`localeCompare` с `numeric: true`). Сортировка по расширению (.jpg перед .png) была опробована, но давала неверный порядок слайдов — не используется.
