...
Правила:
- "questionType" — "choice" если есть варианты A/B/C/D, "text" если вопрос с открытым ответом (без вариантов)
- "correctIndex" — индекс правильного ответа (0-3) для choice-вопросов, -1 для text-вопросов
- "correctAnswer" — текст правильного ответа. Для choice — текст правильного варианта, для text — правильный ответ целиком
- "options" — массив из 4 строк (без префиксов A/B/C/D) для choice, пустой массив [] для text
- "explanation" — ПОЛНЫЙ текст объяснения, включая ВСЕ строки. Если объяснение занимает несколько строк/абзацев — объедини их через \\n. НЕ обрезай текст, бери ВСЁ до следующего вопроса. null если объяснения нет
