...
- **Усечение текстовых ответов**: если вопрос имеет N правильных ответов (через `,`), принимаются только первые N вариантов от пользователя (разделённых `,` или `\n`) — предотвращает перебор всех вариантов
- **Оптимизация broadcast**: `quizId` берётся из уже загруженного объекта вопроса (убрана лишняя SQL-запрос)
- **Логика `weight` в подсчёте очков**: `getResults()` и `getTeamDetails()` теперь используют `q.weight` / `d.weight` для `choice` вопросов (вместо захардкоженного `1`) — вопросы с весом > 1 теперь правильно влияют на рейтинг
- **Edge case LLM**: если `correctAnswers.length === 0`, LLM не вызывается, возвращаются нулевые баллы (предотвращение деления на ноль в промпте)
- **Промпт LLM**: добавлено правило "считать только первые N вариантов"
- **Bot ws-listener**: для текстовых вопросов с несколькими правильными ответами бот подсказывает "Перечисли X ответа через запятую"
