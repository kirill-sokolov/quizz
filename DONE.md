...
**Покрытие (каждый тест создаёт квиз, переводит в нужное состояние, проверяет TV, удаляет квиз):**

- `lobby/regClosed` → TVRules: текст "Правила квиза" виден
- `lobby/regOpen` → TVLobby: заголовок "Регистрация команд" + QR-код (`img[alt="QR код бота"]`) виден
- `playing/question` → TVQuestion: `.tv-screen .bg-stone-900` виден, нет крэша
- `playing/timer` → TVTimer: элемент с `tabular-nums` виден, значение 0–30
- `playing/answer` → TVAnswer: `.tv-screen .bg-stone-900` виден, нет крэша
- `finished/results` → TVResults: заголовок "Итоги квиза" виден (через WebSocket quiz_finished)

**Ключевые технические детали:**
- `finishGame()` устанавливает `quiz.joinCode=null` в БД → TV не может найти квиз по коду после финиша
- Решение для теста results: открыть TV страницу ДО вызова `finish()` — страница получает `quiz_finished` через WebSocket и переключается на TVResults без перезагрузки
- `joinCode` сохраняется из ответа `startGame()` до всех дальнейших API вызовов
- Консольные ошибки фильтруются через `isExpectedError()`: WebSocket, Video autoplay, AudioContext, Failed to load resource — всё ожидаемо при работе без полной аутентификации браузера
- `startGame()` из fixtures возвращает `unknown` — кастуется к `{ joinCode: string }` для типобезопасности

**Запуск:** `cd apps/web && npm run e2e -- e2e/tv-slides.spec.ts`

**Файл:** `apps/web/e2e/tv-slides.spec.ts` (новый)

---

## 2026-02-27: Фронтенд тесты — Stage 3B: Full game flow E2E

### ✅ 3B: `game-flow.spec.ts` — 1 тест, зелёный (3.4s)

**Что тестирует:**

Два browser context (Admin + TV) работают параллельно:
1. Admin navigates to game page → auto-starts → TV: "Правила квиза" (TVRules)
2. Admin: "Открыть регистрацию" → TV: "Регистрация команд" (TVLobby)
3. TestBotsPanel UI: добавить 1 бота → "Начать квиз" становится активным
4. Admin: "Начать квиз" → TV: вопрос-слайд (lobby исчезает)
5. Admin: "▶" → таймер-слайд → TV: countdown виден (`[class*='180px']`)
6. Бот авто-отвечает за ~1с → "▶" снова включается
7. Admin: "▶" → ответ-слайд → TV: countdown пропадает
8. Admin: "Завершить квиз" → TV: "🏆 Итоги квиза" (TVResults)
9. Admin: "Показать следующее место" → TV: "🥇" появляется

**Технические детали:**
- Бот добавляется через TestBotsPanel UI ("Добавить"), а не прямым API-вызовом — TestBotsPanel вызывает `onUpdate()` → `load()`, что мгновенно обновляет список команд
- TVTimer детектируется через `[class*='180px']` (Tailwind JIT-class `text-[180px]`)
- TVRules fallback: при отсутствии `rulesImageUrl` выводится "Правила квиза"
- Cleanup: `deleteQuiz` в `afterAll` каскадно удаляет всё

**Файл:** `apps/web/e2e/game-flow.spec.ts` (новый)
